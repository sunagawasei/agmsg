...
@test "type-registry: cursor manifest declares headless capability" {
  ...
  [ "$(g cursor spawnable)" = yes ]
  [ "$(g cursor headless)" = yes ]
  [ "$(g cursor cli)" = cursor-agent ]
}
...
@test "type-registry: refactored scripts hardcode no per-type branch" {
  ...
  run bash -c "sed 's/#.*//' '$SCRIPTS/whoami.sh' | grep -nE '$types' | grep -vE 'echo \"claude-code\"|AGENT_TYPE\" = \"claude-code\"|type=claude-code' || true"
