...
@test "dispatch: ask blocks then times out (exit 2) when no reply arrives" {
  run bash "$SCRIPTS/dispatch.sh" --type codex --project "$PROJECT_ALICE" --team demo --agent alice -- ask bob "ping" --timeout 1 --interval 1
  [ "$status" -eq 2 ]
...
@test "dispatch: ask returns the reply when bob replies to alice" {
  ...
  run bash "$SCRIPTS/dispatch.sh" --type codex --project "$PROJECT_ALICE" --team demo --agent alice -- ask bob "ping" --timeout 5 --interval 1
  wait
  [ "$status" -eq 0 ]
  [[ "$output" =~ "status=reply" ]]
  [[ "$output" =~ "pong" ]]
...
@test "dispatch: ask separates --timeout from a multi-word message body" {
  ( sleep 1; bash "$SCRIPTS/send.sh" demo bob alice "pong" >/dev/null ) &
  run bash "$SCRIPTS/dispatch.sh" --type codex --project "$PROJECT_ALICE" --team demo --agent alice -- ask bob check the server please --timeout 5 --interval 1
  wait
  [ "$status" -eq 0 ]
  [[ "$output" =~ "status=reply" ]]
  run bash "$SCRIPTS/dispatch.sh" --type codex --project "$PROJECT_ALICE" --team demo -- history
...
@test "dispatch: ask keeps a --timeout literal inside the message body (only trailing options are peeled)" {
  ( sleep 1; bash "$SCRIPTS/send.sh" demo bob alice "pong" >/dev/null ) &
  run bash "$SCRIPTS/dispatch.sh" --type codex --project "$PROJECT_ALICE" --team demo --agent alice -- ask bob set --timeout 5 in the config --timeout 5 --interval 1
  wait
  [ "$status" -eq 0 ]
  [[ "$output" =~ "status=reply" ]]
  run bash "$SCRIPTS/dispatch.sh" --type codex --project "$PROJECT_ALICE" --team demo -- history
...
@test "dispatch: ask rejects an empty message body (exit 2)" {
  run bash "$SCRIPTS/dispatch.sh" --type codex --project "$PROJECT_ALICE" --team demo --agent alice -- ask bob --timeout 1
  [ "$status" -eq 2 ]
...
@test "dispatch: ask -- delimiter sends a body that itself ends with flag-like tokens" {
  ( sleep 1; bash "$SCRIPTS/send.sh" demo bob alice "pong" >/dev/null ) &
  run bash "$SCRIPTS/dispatch.sh" --type codex --project "$PROJECT_ALICE" --team demo --agent alice -- ask bob --timeout 5 -- raw body --interval 9 --timeout 9
  wait
  [ "$status" -eq 0 ]
  [[ "$output" =~ "status=reply" ]]
  run bash "$SCRIPTS/dispatch.sh" --type codex --project "$PROJECT_ALICE" --team demo -- history
