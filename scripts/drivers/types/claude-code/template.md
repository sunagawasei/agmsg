...
2. Run: `~/.agents/skills/__SKILL_NAME__/scripts/spawn.sh <type> <name> --project "$(pwd)" [options]`
   ...
   - **cursor headless**: a headless cursor is always a read-only reviewer in `--project` (no scratch/consultant mode; `--reviewer` is rejected). Read-only is enforced via a scratch `.cursor/cli.json` (config `spawn.cursor_readonly`, default on). Tear down with `despawn --force`.
