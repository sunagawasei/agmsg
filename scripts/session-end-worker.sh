...
#
# <snapshot_path> contains NAME<TAB>RECORD rows for every session-team spawn
# record seen at hook time. Each eligible record is handed to despawn
# --expect-record so we never tear down a worker that a fast lazy-respawn put in
# place AFTER the hook fired.
#
